...
{
    int i;
    for(i=2;i<=number/2;i++) { // am pus de la 2 pentru ca divizorii pleaca de la 2 si am pus mai mic egal ca sa poata intra cand e nevoie 
        if(number%i==0) {
            return FALSE;
        }
    }
...
int main()
{
    ...
    if(readArray(&size,array)==TRUE) {
        computeSum(array,size,&sum); //am pus & deoarece trebuie sa se salveze valoarea sumei in variabila sum si este trimisa adresa la care se va modifica in functie 
        printf("%d",sum);
    }
