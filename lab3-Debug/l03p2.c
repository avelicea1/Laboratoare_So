...
int read_sets(int *set1, int *set2, int *size1, int *size2)
{
    ...
    *size2 = 0;
    ...
    for(i = 1; i <= sz2; i++) {
        if (scanf("%d", &set2[i]) != 1) { // am inlocuit 1 cu 2 pentru a putea citit in vectorul set2
...
float compute(int *set1, int *set2, int size1, int size2)
{
    ...
    while(i <= size1 && j <= size2) { // am pornit de la 1 pentru ca elementele vectorului sunt stocate de la 1 
        //printf("%d %d\n",i,j);
        if(set1[i] == set2[j]) {
            i++;
            common++;
            //printf("in comun %d:",common);
